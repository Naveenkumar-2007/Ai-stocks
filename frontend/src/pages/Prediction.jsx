...
const TradingViewStyleChart = ({ candles = [], sma20 = [], predictions = [], isProfit, currencySymbol = "$" }) => {
  ...
  const width = 980;
  const height = 480;
  const pad = { top: 44, right: 82, bottom: 54, left: 58 };
  ...
  const effectiveStart = viewStart ?? Math.max(0, allValid.length - viewCount);
  const visibleCandles = allValid.slice(effectiveStart, effectiveStart + viewCount);
  ...
  const handlePointerMove = (e) => {
    ...
    if (isDragging) {
      ...
      const newStart = Math.max(0, Math.min(allValid.length - viewCount, dragRef.current.startViewStart + shift));
  ...
  const handleZoomIn = () => {
    ...
    setViewCount(nc);
    const mid = effectiveStart + Math.floor(viewCount / 2);
  ...
  const handleResetView = () => { setViewCount(60); setViewStart(null); };
  ...
  const thumbWidth = Math.max(30, (viewCount / allValid.length) * scrollBarWidth);
  const thumbX = pad.left + (effectiveStart / Math.max(1, allValid.length - viewCount)) * (scrollBarWidth - thumbWidth);
  ...
  return (
    <div className="w-full overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-[#0b1220] shadow-xl dark:shadow-2xl">
      {/* OHLCV Header Bar */}
      <div className="flex items-center gap-3 sm:gap-5 px-3 sm:px-5 py-2 border-b border-slate-200 dark:border-slate-800/80 text-xs sm:text-sm font-mono select-none flex-wrap">
        <span className="text-slate-500 dark:text-slate-400 font-bold text-sm">OHLCV</span>
        {displayCandle && (
          <>
            <span className="text-slate-600 dark:text-slate-500">{fmtDateFull(displayCandle.date)}</span>
            <span className="text-slate-500 dark:text-slate-400">O <span className="text-slate-900 dark:text-white font-semibold">{fmt(displayCandle.open)}</span></span>
            <span className="text-slate-500 dark:text-slate-400">H <span className="text-cyan-600 dark:text-cyan-300 font-semibold">{fmt(displayCandle.high)}</span></span>
            <span className="text-slate-500 dark:text-slate-400">L <span className="text-amber-600 dark:text-amber-300 font-semibold">{fmt(displayCandle.low)}</span></span>
            <span className="text-slate-500 dark:text-slate-400">C <span className="font-bold" style={{ color: displayColor }}>{fmt(displayCandle.close)}</span></span>
            <span style={{ color: displayColor }} className="font-bold">{displayRising ? '+' : ''}{displayChange} ({displayRising ? '+' : ''}{displayChangePct}%)</span>
            <span className="text-slate-600 dark:text-slate-500">Vol <span className="text-slate-800 dark:text-slate-300">{formatVolume(displayCandle.volume)}</span></span>
          </>
        )}
      </div>
      ...
      <svg
        ...
        className="w-full h-[340px] sm:h-[400px] lg:h-[440px] select-none"
      ...
      {/* Controls Bar */}
      <div className="flex items-center justify-between px-3 sm:px-5 py-2 border-t border-slate-800/60 text-xs">
        <div className="flex items-center gap-1.5">
          <button onClick={handleZoomIn} className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-md transition-colors font-mono font-bold">+</button>
          <button onClick={handleZoomOut} className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-md transition-colors font-mono font-bold">−</button>
          <button onClick={handleResetView} className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-400 rounded-md transition-colors text-[10px] font-semibold">RESET</button>
        </div>
        <span className="text-slate-500 font-mono">{visibleCandles.length}/{allValid.length} candles &middot; Drag to scroll &middot; Scroll to zoom</span>
        <div className="flex items-center gap-1.5">
          {[30, 60, 90].map((n) => (
            <button key={n} onClick={() => { setViewCount(n); setViewStart(Math.max(0, allValid.length - n)); }}
              className={`px-2 py-1 rounded-md font-mono text-[10px] font-bold transition-colors ${viewCount === n ? 'bg-cyan-600 text-white' : 'bg-slate-800 hover:bg-slate-700 text-slate-400'}`}>
              {n}D
            </button>
          ))}
          <button onClick={() => { setViewCount(allValid.length); setViewStart(0); }}
            className={`px-2 py-1 rounded-md font-mono text-[10px] font-bold transition-colors ${viewCount >= allValid.length ? 'bg-cyan-600 text-white' : 'bg-slate-800 hover:bg-slate-700 text-slate-400'}`}>
            ALL
          </button>
        </div>
      </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                    <ComposedChart data={[
                      ...
                      <XAxis
                        ...
                        className="text-gray-600 dark:text-gray-400 stroke-gray-400 dark:stroke-gray-600"
                        angle={-45}
                        textAnchor="end"
                        height={65}
                      />
                      ...
                      <Tooltip
                        ...
                        content={({ active, payload }) => {
                          if (active && payload && payload.length) {
                            const data = payload[0].payload;
                            return (
                              <div className="bg-white/95 dark:bg-gray-900/95 p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl backdrop-blur-sm">
                                <p className="text-xs font-bold text-gray-900 dark:text-white mb-2">{data.fullDate}</p>
                                {data.price && (
                      ...
                      <ReferenceLine
                        ...
                        strokeDasharray="3 3"
                        strokeWidth={1.5}
                        label={{ value: 'Current', fontSize: 11, fill: 'currentColor', position: 'right', className: "text-gray-600 dark:text-gray-400" }}
                      />
                    </ComposedChart>
                  </ResponsiveContainer>
                </div>
                        ...
                        content={({ active, payload }) => {
                          if (active && payload && payload.length) {
                            ...
                            return (
                              <div className="bg-white/95 dark:bg-gray-900/95 p-3 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl backdrop-blur-sm">
                                <p className="text-xs font-bold text-gray-900 dark:text-white mb-2">{data.fullDate}</p>
                                <p className="text-xs text-cyan-600 dark:text-cyan-400">Price: <span className="font-bold">{currencySymbol}{data.price.toFixed(2)}</span></p>
